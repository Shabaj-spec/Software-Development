...
public class TimBot {
   ...
   */
  protected void setSpressoSensed(int[] spresso) {
    if (spresso != null && spresso.length == spressoSensed.length) {
      System.arraycopy(spresso, 0, spressoSensed, 0, spresso.length);
    }
  }

  protected void setBotsSensed(boolean[] bots) {
    if (bots != null && bots.length == botsSensed.length) {
      System.arraycopy(bots, 0, botsSensed, 0, bots.length);
    }
  }

  // Refactored senseDistricts method using encapsulated setters
  public void senseDistricts(int[] spresso, boolean[] bots) {
    setSpressoSensed(spresso);
    setBotsSensed(bots);
  }
