...
public class User {
  ...
  public User( String nm ) {
    name = nm;
    // Check if the user already exists, only add if they don't
  ...
  public User unfriend( String f ) {
    User u = users.get( f );
    if (u != null && adj.containsKey(u.name)) {
      adj.remove(u.name);
   ...
   */
  public void leave() {
    // Remove this user from all friends' adjacency lists
    for (User friend : adj.values()) {
      friend.adj.remove(name);
    }

    // Clear this user's adjacency list
    adj.clear();

    // Remove this user from the global list
    users.remove(name);
  }
